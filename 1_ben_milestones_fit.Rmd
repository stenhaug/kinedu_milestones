...
mod_4pl_mc <- mirt(d_mat, 1, itemtype='4PL', verbose=FALSE, TOL = 0.00000001, method = "MCEM")

mod_4pl_mh <- mirt(d_mat, 1, itemtype='4PL', verbose=FALSE, TOL = 0.00000001, method = "MHRM")

coef(mod_4pl_mc)$color_666

coef(mod_4pl_mh)$color_666

coef(mod_4pl_mc, simplify = TRUE)$items$a1

plot(coef(mod_4pl, simplify = TRUE)$items[ , "a1"], coef(mod_4pl_mc, simplify = TRUE)$items[ , "a1"])
```
...
tibble(
  disc_3pl = coef_3pl$a1,
  disc_4pl = coef_4pl$a1,
  item = coef_3pl$item
) %>% 
  ggplot(aes(x = disc_3pl, y = disc_4pl)) +
  geom_point() +
  labs(title = "Comparing discrimination parameters from 3PL vs 4PL")

bind_rows(
  coef_3pl %>% mutate(model = "3PL"),
  coef_4pl %>% mutate(model = "4PL")
) %>% 
  ggplot(aes(x = a1, fill = model)) +
  geom_histogram() +
        ...
        prob = 
            map2_dbl(
                ...
                ~ probtrace(
                    extract.item(
                        get(.x), 
                        which(coef_3pl$item == item) # 3pl and 4pl have same order
                    ), 
                    .y
                )[2]
            )
